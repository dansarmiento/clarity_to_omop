/*
Purpose: This query transforms hospital medication administration data into the OMOP CDM Drug_Exposure table format
Source: Hospital medication administration records (V_RX_CHARGES)
Target: OMOP Drug_Exposure table
*/
...
--------------------------------
-- STAGE_DRUG_EXPOSURE_HSP_V_RX_CHARGES
--------------------------------
SELECT DISTINCT
    -- Core OMOP Fields
    PULL_DRUG_EXPOSURE_HOSP_V_RX_CHARGES.PERSON_ID AS PERSON_ID,
    ...
    -- Source Values
    PULL_DRUG_EXPOSURE_HOSP_V_RX_CHARGES.PULL_MEDICATION_ID::VARCHAR 
        || ':' || PULL_MEDICATION_NAME AS DRUG_SOURCE_VALUE,
    T_DRUG_SOURCE.CONCEPT_ID AS DRUG_SOURCE_CONCEPT_ID,
    PULL_DRUG_EXPOSURE_HOSP_V_RX_CHARGES.PULL_MED_ROUTE_C::VARCHAR 
        || ':' || PULL_ZC_ADMIN_ROUTE_NAME AS ROUTE_SOURCE_VALUE,
    DOSE_UNIT_SOURCE_VALUE AS DOSE_UNIT_SOURCE_VALUE,

    -- Custom Non-OMOP Fields
    'DRUG_EXPOSURE_HSP_V_RX_CHARGES' AS ETL_MODULE,
    VISIT_OCCURRENCE.phi_PAT_ID AS PAT_ID,
    VISIT_OCCURRENCE.phi_MRN_CPI AS MRN_CPI,
    VISIT_OCCURRENCE.phi_CSN_ID AS CSN_ID,
    PULL_DRUG_EXPOSURE_HOSP_V_RX_CHARGES.PULL_MEDICATION_ID AS MEDICATION_ID

FROM CARE_RES_OMOP_DEV2_WKSP.OMOP_PULL.PULL_DRUG_EXPOSURE_HOSP_V_RX_CHARGES AS PULL_DRUG_EXPOSURE_HOSP_V_RX_CHARGES

    -- Join to get visit information
    INNER JOIN CARE_RES_OMOP_DEV2_WKSP.OMOP.VISIT_OCCURRENCE_RAW AS VISIT_OCCURRENCE
        ON PULL_DRUG_EXPOSURE_HOSP_V_RX_CHARGES.PULL_CSN_ID = VISIT_OCCURRENCE.phi_CSN_ID

    -- Join to get provider information
    LEFT JOIN CARE_RES_OMOP_DEV2_WKSP.OMOP.PROVIDER_RAW AS PROVIDER
        ON PULL_DRUG_EXPOSURE_HOSP_V_RX_CHARGES.PROVIDER_SOURCE_VALUE = PROVIDER.PROVIDER_SOURCE_VALUE

    -- Join to get RxNorm codes
    INNER JOIN CARE_RES_OMOP_DEV2_WKSP.OMOP_STAGE.TOP_HSP_RXNORM_V_RX_CHARGES AS TOP_RXNORM
        ON PULL_DRUG_EXPOSURE_HOSP_V_RX_CHARGES.PULL_MEDICATION_ID = TOP_RXNORM.PULL_MEDICATION_ID
    ...
    LEFT JOIN cte__SOURCE_TO_CONCEPT_MAP_ROUTE AS SOURCE_TO_CONCEPT_MAP_ROUTE
        ON PULL_DRUG_EXPOSURE_HOSP_V_RX_CHARGES.PULL_MED_ROUTE_C::VARCHAR = SOURCE_TO_CONCEPT_MAP_ROUTE.SOURCE_CODE::VARCHAR
...
WHERE PULL_DRUG_EXPOSURE_HOSP_V_RX_CHARGES.DRUG_EXPOSURE_START_DATE IS NOT NULL