--  sh_flwsht_meas_temperature_source.sql
--> export file name pattern: sh_flwsht_meas_temperature_source
...
WHERE --DATEDIFF(YEAR,FSM.RECORDED_TIME,GETDATE()) <= 1 AND 
      (FS.FLO_MEAS_NAME  LIKE ('%TEMPERATURE%') OR FS.FLO_MEAS_NAME  LIKE ('%TEMP%'))
      AND 
      FS.FLO_MEAS_NAME  NOT LIKE ('%ATTEMPT%')
