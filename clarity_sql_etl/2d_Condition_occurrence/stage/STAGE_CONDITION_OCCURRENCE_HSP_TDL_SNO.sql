/***************************************************************
 * Script: STAGE_CONDITION_OCCURRENCE_HSP_TDL_SNO
 * Description: Extracts and transforms condition occurrence data 
 * from hospital TDL SNOMED records into OMOP CDM format
 ***************************************************************/

SELECT DISTINCT
    PULL_CONDITION_OCCURRENCE_HOSP_TDL_SNO.PERSON_ID,
    -- Standard OMOP Fields
    T_CONDITION_CONCEPT.CONCEPT_ID                                      AS CONDITION_CONCEPT_ID,
    PULL_CONDITION_OCCURRENCE_HOSP_TDL_SNO.CONDITION_START_DATE        AS CONDITION_START_DATE,
    PULL_CONDITION_OCCURRENCE_HOSP_TDL_SNO.CONDITION_START_DATETIME    AS CONDITION_START_DATETIME,
    PULL_CONDITION_OCCURRENCE_HOSP_TDL_SNO.CONDITION_END_DATE          AS CONDITION_END_DATE,
    PULL_CONDITION_OCCURRENCE_HOSP_TDL_SNO.CONDITION_END_DATETIME      AS CONDITION_END_DATETIME,
    32817                                                              AS CONDITION_TYPE_CONCEPT_ID,  -- EHR Detail
    ...
    'STAGE_CONDITION_OCCURRENCE_HSP_TDL_SNO'                          AS ETL_MODULE,
...
FROM CARE_RES_OMOP_DEV2_WKSP.OMOP_PULL.PULL_CONDITION_OCCURRENCE_HOSP_TDL_SNO AS PULL_CONDITION_OCCURRENCE_HOSP_TDL_SNO

    -- Join to get Visit Information
    INNER JOIN CARE_RES_OMOP_DEV2_WKSP.OMOP.VISIT_OCCURRENCE_RAW AS VISIT_OCCURRENCE
        ON PULL_CONDITION_OCCURRENCE_HOSP_TDL_SNO.PULL_CSN_ID = VISIT_OCCURRENCE.phi_CSN_ID

    -- Join to get Provider Information
    LEFT OUTER JOIN CARE_RES_OMOP_DEV2_WKSP.OMOP.PROVIDER_RAW AS PROVIDER
        ON PULL_CONDITION_OCCURRENCE_HOSP_TDL_SNO.PROVIDER_SOURCE_VALUE = PROVIDER.PROVIDER_SOURCE_VALUE

    -- Concept Mapping Joins
    LEFT JOIN CARE_RES_OMOP_DEV2_WKSP.OMOP_STAGE.T_SNO_SOURCE AS T_SNO_SOURCE
        ON PULL_CONDITION_OCCURRENCE_HOSP_TDL_SNO.PULL_SNOMED_CODE = T_SNO_SOURCE.CONCEPT_CODE
