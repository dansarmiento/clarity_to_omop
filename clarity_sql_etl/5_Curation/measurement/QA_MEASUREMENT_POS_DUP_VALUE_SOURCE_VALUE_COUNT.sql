---------------------------------------------------------------------
-- QA_MEASUREMENT_DUPLICATES_COUNT_VALUE_SOURCE_VALUE
-- Purpose: 
...
- METRIC_FIELD: Specific field being checked (VALUE_SOURCE_VALUE)
...
1. Creates a CTE (TMP_DUPES) that:
   - Groups measurements by all relevant fields
   - Counts occurrences of identical measurements
   - Filters for groups with more than one occurrence
2. Main query summarizes duplicate counts and adds metadata
3. Only includes records where:
   - MEASUREMENT_CONCEPT_ID is valid (not 0 or NULL)
