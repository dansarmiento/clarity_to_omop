/*
Purpose: This query transforms anesthesia medication data into the OMOP CDM Drug_Exposure table format
Source: PULL_DRUG_EXPOSURE_ANES_RXNORM and related tables
Target: DRUG_EXPOSURE table (OMOP CDM)
*/

-- CTE to get route information from source to concept mapping
...
-- Main query to transform anesthesia medication data
SELECT DISTINCT
...
-- Join chain to get drug concepts
INNER JOIN CARE_RES_OMOP_DEV2_WKSP.OMOP_STAGE.TOP_ANES_RXNORM AS TOP_RXNORM
...
INNER JOIN (
    SELECT 
        ...
        CONCEPT_CLASS_ID,
        THEORDER
    FROM CARE_RES_OMOP_DEV2_WKSP.OMOP_STAGE.TOP_ANES_RXNORM AS TOP_RXNORM
    GROUP BY 
        PULL_MEDICATION_ID,
        CONCEPT_CLASS_ID,
        THEORDER
