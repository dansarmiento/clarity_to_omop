...
---***************************************
    FROM {{ref('DRUG_EXPOSURE_RAW')}} AS  d
        JOIN {{ref('CONCEPT_ANCESTOR_stg')}} AS  ca ON ca.descendant_concept_id = d.drug_concept_id
        JOIN {{ref('CONCEPT_stg')}} AS  c ON ca.ancestor_concept_id = c.concept_id
