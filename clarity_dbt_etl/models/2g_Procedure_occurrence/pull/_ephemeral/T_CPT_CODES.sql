...
FROM {{ ref('CLARITY_EAP_stg')}} AS EAP
    LEFT JOIN {{ ref('LINKED_PERFORMABLE_stg')}} AS LINKED_PERFORMABLE
        ON EAP.PROC_ID = LINKED_PERFORMABLE.PROC_ID
    LEFT JOIN {{ ref('CLARITY_EAP_stg')}} AS EAP2
...
SELECT DISTINCT EAP.PROC_ID AS PROC_ID
    ...
    END AS PROC_NAME
FROM {{ ref('CLARITY_EAP_stg')}} AS EAP
    LEFT JOIN {{ ref('LINKED_CHARGEABLES_stg')}} AS LINKED_CHARGEABLES
