...
--
SELECT DISTINCT
      PULL_OBSERVATION_HOSP_FLOWSHEET.PERSON_ID
      ,COALESCE(
            SOURCE_TO_CONCEPT_MAP_FLOWSHEET_MEAS.TARGET_CONCEPT_ID
      ...
      ,PULL_OBSERVATION_HOSP_FLOWSHEET.PULL_FLO_MEAS_ID::VARCHAR
            ||':'|| PULL_OBSERVATION_HOSP_FLOWSHEET.PULL_FLO_MEAS_NAME       AS OBSERVATION_SOURCE_VALUE
	...
	-------- Non-OMOP Fields ------------
      ,'OBSERVATION_HSP_SDOH_FLOWSHEET' 				                AS ETL_MODULE
...
FROM {{ref('PULL_OBSERVATION_HOSP_FLOWSHEET')}} AS PULL_OBSERVATION_HOSP_FLOWSHEET

INNER JOIN {{ ref('SOURCE_TO_CONCEPT_MAP_SDOH')}} AS SOURCE_TO_CONCEPT_MAP_FLOWSHEET_MEAS
      ON PULL_OBSERVATION_HOSP_FLOWSHEET.PULL_FLO_MEAS_ID = SOURCE_TO_CONCEPT_MAP_FLOWSHEET_MEAS.SOURCE_CODE

LEFT JOIN {{ ref('SOURCE_TO_CONCEPT_MAP_SDOH_ANSWERS')}} AS SOURCE_TO_CONCEPT_MAP_VALUE
            ON PULL_OBSERVATION_HOSP_FLOWSHEET.PULL_MEAS_VALUE ||'_'|| PULL_CAT_INI || '_' || PULL_CAT_ITEM = SOURCE_TO_CONCEPT_MAP_VALUE.SOURCE_CODE

INNER JOIN {{ref('VISIT_OCCURRENCE_RAW')}} VISIT_OCCURRENCE
      ON PULL_OBSERVATION_HOSP_FLOWSHEET.PULL_CSN_ID = VISIT_OCCURRENCE.phi_CSN_ID

	LEFT JOIN {{ref('PROVIDER_RAW')}} AS PROVIDER
		ON PULL_OBSERVATION_HOSP_FLOWSHEET.PROVIDER_SOURCE_VALUE = PROVIDER.PROVIDER_SOURCE_VALUE
