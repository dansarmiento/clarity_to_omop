---------------------------------------------------------------------
-- QA_DRUG_EXPOSURE_POS_DUP_VERBATIM_END_DATE_DETAIL
-- Purpose: Identifies possible duplicate drug exposure records based on matching values across multiple fields
---------------------------------------------------------------------
...
1. CTE TMP_DUPES finds groups of records that match on:
   ...
   - DRUG_EXPOSURE_START_DATETIME
   - DRUG_EXPOSURE_END_DATETIME
   - Other drug exposure details (stop reason, refills, quantity, etc)
   
2. Main query joins back to DRUG_EXPOSURE_RAW to get the specific DRUG_EXPOSURE_IDs
   for records identified as potential duplicates

3. COALESCE is used throughout to handle NULL values in comparison fields
