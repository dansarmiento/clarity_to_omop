---------------------------------------------------------------------
-- QA_DRUG_EXPOSURE_POS_DUP_STOP_REASON_DETAIL
-- Purpose:
-- This query identifies possible duplicate records in the DRUG_EXPOSURE_RAW table by comparing multiple fields
-- and generates a quality assurance report.
---------------------------------------------------------------------
...
- METRIC_FIELD: Field being analyzed for QA (STOP_REASON)
...
Logic:
1. TMP_DUPES CTE:
   - Groups records by multiple fields to identify duplicate entries
   - Filters out records with invalid DRUG_CONCEPT_ID and PERSON_ID
   - Counts occurrences of identical records

2. Main Query:
   - Joins the identified duplicates back to the original table
   - Uses COALESCE to handle NULL values in comparison fields
