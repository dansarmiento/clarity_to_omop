...
*              classifications with a defined order of precedence
* Tables: 
*   - CARE_RES_OMOP_DEV2_WKSP.OMOP_PULL.PULL_DRUG_EXPOSURE_HOSP_RXNORM
...
SELECT DISTINCT 
    ...
    C.THEORDER
FROM 
    CARE_RES_OMOP_DEV2_WKSP.OMOP_PULL.PULL_DRUG_EXPOSURE_HOSP_RXNORM AS PULL_DRUG_EXPOSURE_HOSP_RXNORM
    INNER JOIN (
        SELECT 
            CONCEPT_CODE,
            CONCEPT_CLASS_ID,
            -- Order of precedence for different medication concept classes
            CASE
                ...
                -- Classifications with undefined order (set to lowest priority)
    ...
    ) C ON PULL_DRUG_EXPOSURE_HOSP_RXNORM.PULL_RXNORM_CODE = CONCEPT_CODE;