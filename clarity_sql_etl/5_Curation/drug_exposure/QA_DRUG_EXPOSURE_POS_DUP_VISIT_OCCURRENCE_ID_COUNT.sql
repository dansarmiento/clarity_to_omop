...
- METRIC_FIELD: Field being checked for duplicates (VISIT_OCCURRENCE_ID)
...
1. Creates a CTE (TMP_DUPES) that:
   - Groups records by multiple fields
   - Counts occurrences of identical records
   - Filters for records with count > 1
   - Excludes records with null or zero DRUG_CONCEPT_ID and PERSON_ID

2. Main query:
   - Summarizes duplicate counts
   - Provides metadata about the QA process
