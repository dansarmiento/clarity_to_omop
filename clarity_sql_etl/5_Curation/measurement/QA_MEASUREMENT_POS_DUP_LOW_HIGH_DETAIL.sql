---------------------------------------------------------------------
-- -- QA_MEASUREMENT_DUPLICATES_DETAIL_LOW_HIGH
-- Purpose: 
...
- STANDARD_DATA_TABLE: Indicates the source table (MEASUREMENT)
- QA_METRIC: Type of quality check (POSSIBLE_DUPLICATE)
- METRIC_FIELD: Specific field being checked (RANGE_LOW_HIGH)
...
1. TMP_DUPES CTE:
   ...
   - Identifies records where exact matches occur more than once
   - Excludes records with invalid MEASUREMENT_CONCEPT_ID or PERSON_ID

2. Main Query:
   - Joins temporary duplicates back to original table
   - Uses COALESCE to handle NULL values in join conditions
   - Returns specific duplicate measurement IDs for investigation

Note: This query is part of a QA process to identify potentially duplicate measurement records
