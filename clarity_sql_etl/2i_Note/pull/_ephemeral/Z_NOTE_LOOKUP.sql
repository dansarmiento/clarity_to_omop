...
-- pulls a list of available notes
    ...
    FROM
    CARE_RES_OMOP_DEV2_WKSP.OMOP_CLARITY.HNO_INFO_stg AS HNO_INFO
    INNER JOIN CARE_RES_OMOP_DEV2_WKSP.OMOP_CLARITY.ZC_NOTE_TYPE_IP_stg AS ZC_NOTE_TYPE_IP
        ON HNO_INFO.IP_NOTE_TYPE_C = ZC_NOTE_TYPE_IP.TYPE_IP_C
    INNER JOIN CARE_RES_OMOP_DEV2_WKSP.OMOP_CLARITY.NOTE_ENC_INFO_stg AS NOTE_ENC_INFO
        ON HNO_INFO.NOTE_ID = NOTE_ENC_INFO.NOTE_ID
    -- INNER JOIN CARE_RES_OMOP_DEV2_WKSP.OMOP_CLARITY.HNO_NOTE_TEXT_stg AS HNO_NOTE_TEXT
    ...
    -- WHERE HNO_INFO.PAT_ID IN (SELECT EHR_PATIENT_ID from CARE_RES_OMOP_DEV2_WKSP.OMOP.PATIENT_DRIVER)
