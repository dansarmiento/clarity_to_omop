...
SELECT DISTINCT
    {{ sequence_get_nextval() }}::NUMBER(28,0)   AS DEVICE_EXPOSURE_ID
    ...
    , DEVICE_SOURCE_VALUE::VARCHAR(250)           AS DEVICE_SOURCE_VALUE --Human readable Source Value use src_VALUE_ID below to link
...
FROM
(SELECT PERSON_ID, DEVICE_CONCEPT_ID, DEVICE_EXPOSURE_START_DATE
     ...
     , 'ORDER_PROC' as SRC_TABLE, 'PROC_ID' AS SRC_FIELD, STAGE_PROC_ID::INTEGER AS SRC_VALUE_ID
FROM
     {{ref('STAGE_DEVICE_EXPOSURE_HSP_BLOOD')}} AS STAGE_DEVICE_EXPOSURE_HSP_BLOOD
UNION ALL
SELECT PERSON_ID, DEVICE_CONCEPT_ID, DEVICE_EXPOSURE_START_DATE
     , DEVICE_EXPOSURE_START_DATETIME, DEVICE_EXPOSURE_END_DATE, DEVICE_EXPOSURE_END_DATETIME
     , DEVICE_TYPE_CONCEPT_ID, UNIQUE_DEVICE_ID, PRODUCTION_ID, QUANTITY, PROVIDER_ID, VISIT_OCCURRENCE_ID
...
FROM
     {{ref('STAGE_DEVICE_EXPOSURE_HSP_LDA')}} AS STAGE_DEVICE_EXPOSURE_HSP_LDA
UNION ALL
SELECT PERSON_ID, DEVICE_CONCEPT_ID, DEVICE_EXPOSURE_START_DATE
     , DEVICE_EXPOSURE_START_DATETIME, DEVICE_EXPOSURE_END_DATE, DEVICE_EXPOSURE_END_DATETIME
     , DEVICE_TYPE_CONCEPT_ID, UNIQUE_DEVICE_ID, PRODUCTION_ID, QUANTITY, PROVIDER_ID, VISIT_OCCURRENCE_ID
     ...
     , 'IP_FLO_GP_DATA' as SRC_TABLE, 'FLO_MEAS_ID' AS SRC_FIELD, STAGE_FLO_MEAS_ID::INTEGER AS SRC_VALUE_ID
FROM
     {{ref('STAGE_DEVICE_EXPOSURE_ANES_LDA')}} AS V_DEVICE_EXPOSURE_ANES_LDA
     ) A
