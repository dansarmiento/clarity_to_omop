/*
Purpose: Transform and load hospital drug exposure data with RxNorm mappings
Source: PULL_DRUG_EXPOSURE_HOSP_RXNORM
Target: STAGE_DRUG_EXPOSURE_HSP_RXNORM
*/

-- Common Table Expression for Route Mapping
...
SELECT DISTINCT
    ...
    PULL_DRUG_EXPOSURE_HOSP_RXNORM.PERSON_ID                     AS PERSON_ID,
    ...
    PULL_DRUG_EXPOSURE_HOSP_RXNORM.PULL_MEDICATION_ID         AS STAGE_MEDICATION_ID

FROM CARE_RES_OMOP_DEV2_WKSP.OMOP_PULL.PULL_DRUG_EXPOSURE_HOSP_RXNORM AS PULL_DRUG_EXPOSURE_HOSP_RXNORM

    -- Join to get Visit Information
    INNER JOIN CARE_RES_OMOP_DEV2_WKSP.OMOP.VISIT_OCCURRENCE_RAW AS VISIT_OCCURRENCE
        ON PULL_DRUG_EXPOSURE_HOSP_RXNORM.PULL_CSN_ID = VISIT_OCCURRENCE.phi_CSN_ID

    -- Join to get Provider Information
    LEFT JOIN CARE_RES_OMOP_DEV2_WKSP.OMOP.PROVIDER_RAW AS PROVIDER
        ON PULL_DRUG_EXPOSURE_HOSP_RXNORM.PROVIDER_SOURCE_VALUE = PROVIDER.PROVIDER_SOURCE_VALUE

    -- Join to get RxNorm Mappings
    INNER JOIN CARE_RES_OMOP_DEV2_WKSP.OMOP_STAGE.TOP_HSP_RXNORM AS TOP_RXNORM
        ON PULL_DRUG_EXPOSURE_HOSP_RXNORM.PULL_MEDICATION_ID = TOP_RXNORM.PULL_MEDICATION_ID

    -- Join to get Drug Source Concepts
    ...
    -- Join to get Route Mappings
    LEFT JOIN cte__SOURCE_TO_CONCEPT_MAP_ROUTE AS SOURCE_TO_CONCEPT_MAP_ROUTE
        ON PULL_DRUG_EXPOSURE_HOSP_RXNORM.PULL_MED_ROUTE_C::VARCHAR = SOURCE_TO_CONCEPT_MAP_ROUTE.SOURCE_CODE::VARCHAR

    -- Subquery to Remove Duplicates
    INNER JOIN (
        SELECT 
            MIN(TOP_RXNORM.PULL_RXNORM_CODES_LINE) AS FIRSTLINE,
            TOP_RXNORM.PULL_MEDICATION_ID,
            MIN(CONCEPT_CODE) AS CONCEPT_CODE,
            CONCEPT_CLASS_ID,
            THEORDER
        FROM CARE_RES_OMOP_DEV2_WKSP.OMOP_STAGE.TOP_HSP_RXNORM AS TOP_RXNORM
        GROUP BY 
            TOP_RXNORM.PULL_MEDICATION_ID,
            CONCEPT_CLASS_ID,
            THEORDER
...
WHERE PULL_DRUG_EXPOSURE_HOSP_RXNORM.DRUG_EXPOSURE_START_DATE IS NOT NULL