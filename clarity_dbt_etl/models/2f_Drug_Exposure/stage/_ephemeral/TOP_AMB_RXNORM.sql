...
--BEGIN cte__TOP_AMB_RXNORM
	SELECT DISTINCT PULL_DRUG_EXPOSURE_AMB_RXNORM.PULL_MEDICATION_ID
		,PULL_RXNORM_CODES_LINE
		,C.CONCEPT_CODE
		,C.CONCEPT_CLASS_ID
		,C.THEORDER

    FROM {{ref('PULL_DRUG_EXPOSURE_AMB_RXNORM')}} AS PULL_DRUG_EXPOSURE_AMB_RXNORM
    INNER JOIN (
        SELECT CONCEPT_CODE
            ,CONCEPT_CLASS_ID
        ...
        FROM {{ref('CONCEPT_stg')}} AS CONCEPT
        ...
        ON PULL_DRUG_EXPOSURE_AMB_RXNORM.PULL_RXNORM_CODE = CONCEPT_CODE
    -- WHERE PULL_DRUG_EXPOSURE_AMB_RXNORM.RXNORM_TERM_TYPE_C <> 2 --2 - PRECISE INGREDIENT
--END cte__TOP_AMB_RXNORM
--
