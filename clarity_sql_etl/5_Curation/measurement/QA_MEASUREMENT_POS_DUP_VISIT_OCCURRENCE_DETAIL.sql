---------------------------------------------------------------------
-- QA_MEASUREMENT_DUPLICATES_DETAIL_VISIT_OCCURRENCE
-- Purpose: 
-- This query identifies potential duplicate measurements in the MEASUREMENT_RAW table.
---------------------------------------------------------------------
...
Logic:
1. TMP_DUPES CTE:
   - Groups measurements by key fields
   - Identifies records where multiple entries exist for the same combination of:
     * PERSON_ID
     * MEASUREMENT_CONCEPT_ID
     * MEASUREMENT_DATETIME
     * VALUE_AS_NUMBER
...
2. Main Query:
   - Joins the duplicate records back to the original table
   - Uses COALESCE to handle NULL values in the join conditions
   - Returns specific measurement IDs that are potentially duplicated
