...
FROM {{ref('PULL_DRUG_EXPOSURE_HOSP_V_RX_CHARGES')}} AS PULL_DRUG_EXPOSURE_HOSP_V_RX_CHARGES
	...
	INNER JOIN {{ref('TOP_HSP_RXNORM_V_RX_CHARGES')}} AS TOP_RXNORM
	...
	INNER JOIN -- THIS REMOVES THE DUPLICATES
		(
		SELECT MIN(TOP_RXNORM.PULL_RXNORM_CODES_LINE) AS FIRSTLINE
			,TOP_RXNORM.PULL_MEDICATION_ID
			,MIN(CONCEPT_CODE) AS CONCEPT_CODE
			,CONCEPT_CLASS_ID
			,THEORDER
		FROM {{ref('TOP_HSP_RXNORM')}} AS TOP_RXNORM
		GROUP BY TOP_RXNORM.PULL_MEDICATION_ID
			,CONCEPT_CLASS_ID
			,THEORDER
