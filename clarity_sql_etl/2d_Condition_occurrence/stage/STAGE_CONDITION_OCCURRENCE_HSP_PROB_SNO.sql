/***************************************************************
 * Script: STAGE_CONDITION_OCCURRENCE_HSP_PROB_SNO
 * Description: Extracts and transforms hospital problem list data 
 * with SNOMED codes into OMOP CDM format
 ***************************************************************/

SELECT DISTINCT
    PULL_CONDITION_OCCURRENCE_HOSP_PROB_SNO.PERSON_ID,
    T_CONDITION_CONCEPT.CONCEPT_ID AS CONDITION_CONCEPT_ID,
    PULL_CONDITION_OCCURRENCE_HOSP_PROB_SNO.CONDITION_START_DATE AS CONDITION_START_DATE,
    PULL_CONDITION_OCCURRENCE_HOSP_PROB_SNO.CONDITION_START_DATETIME AS CONDITION_START_DATETIME,
    PULL_CONDITION_OCCURRENCE_HOSP_PROB_SNO.CONDITION_END_DATE AS CONDITION_END_DATE,
    PULL_CONDITION_OCCURRENCE_HOSP_PROB_SNO.CONDITION_END_DATETIME AS CONDITION_END_DATETIME,
    32817 AS CONDITION_TYPE_CONCEPT_ID,  -- Standard concept for EHR problem list entry
    STOP_REASON AS STOP_REASON,
    PROVIDER.PROVIDER_ID AS PROVIDER_ID,
    VISIT_OCCURRENCE.VISIT_OCCURRENCE_ID AS VISIT_OCCURRENCE_ID,
    NULL AS VISIT_DETAIL_ID,  -- Not currently implemented
    
    -- Concatenate ICD code and SNOMED concept name, limited to 50 characters
    ...
    -- Map condition status to standard concepts
    CASE 
    ...
    'STAGE_CONDITION_OCCURRENCE_HSP_PROB_SNO' AS ETL_MODULE,
...
FROM CARE_RES_OMOP_DEV2_WKSP.OMOP_PULL.PULL_CONDITION_OCCURRENCE_HOSP_PROB_SNO 
    AS PULL_CONDITION_OCCURRENCE_HOSP_PROB_SNO

    -- Join to get visit information
    INNER JOIN CARE_RES_OMOP_DEV2_WKSP.OMOP.VISIT_OCCURRENCE_RAW AS VISIT_OCCURRENCE
        ON PULL_CONDITION_OCCURRENCE_HOSP_PROB_SNO.PULL_CSN_ID = VISIT_OCCURRENCE.phi_CSN_ID

    -- Join to get provider information
    LEFT OUTER JOIN CARE_RES_OMOP_DEV2_WKSP.OMOP.PROVIDER_RAW AS PROVIDER
        ON PULL_CONDITION_OCCURRENCE_HOSP_PROB_SNO.PROVIDER_SOURCE_VALUE = PROVIDER.PROVIDER_SOURCE_VALUE

    -- Concept mapping joins
    LEFT JOIN CARE_RES_OMOP_DEV2_WKSP.OMOP_STAGE.T_SNO_SOURCE AS T_SNO_SOURCE
        ON PULL_CONDITION_OCCURRENCE_HOSP_PROB_SNO.PULL_SNOMED_CODE = T_SNO_SOURCE.CONCEPT_CODE
